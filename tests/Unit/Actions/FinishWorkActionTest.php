...
class FinishWorkActionTest extends TestCase
{
    ...
    private function createInProgressServiceOrder(): ServiceOrder
    {
        ...
        $serviceOrder = $approveAction($serviceOrder->id, $user->id);

        $startAction = new StartWorkAction($domain);

        return $startAction($serviceOrder->id, $user->id);
    }
     ...
     * @throws Throwable
     */
    public function test_finishes_work_moves_to_waiting_payment(): void
    {
        ...
        $action = new FinishWorkAction($domain);
     ...
     * @throws BindingResolutionException
     */
    public function test_throws_exception_when_not_in_progress(): void
    {
        $client = Client::factory()->create();
        $vehicle = Vehicle::factory()->create();
        $user = User::factory()->create();
